...
void	SkyDrawing::draw(QPainter& painter, QSize& size) {
	...
	PlanetDrawing::radius(_radius / 180.);
	if (show_sun()) {
		drawSun(painter);
	}
	if (show_moon()) {
...
/**
 * \brief compute the position of the planet in the image
 *
 * This method overrides the one in PlanetDrawing
 *
 * \param pos	position of the planet
 */
QPointF	SkyDrawing::position(const astro::RaDec& pos) {
	astro::AzmAlt	Position = convert(pos);
	if (Position.alt().radians() < 0) {
		throw std::runtime_error("object below horizon");
	}
