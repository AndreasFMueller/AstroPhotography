...
#define	RP	GuiderPort::RAPLUS
#define	RM	GuiderPort::RAMINUS
#define	DP	GuiderPort::DECPLUS
#define	DM	GuiderPort::DECMINUS
...
void	display(astro::camera::GuiderPortPtr guiderport,
...
void	prog0(astro::camera::GuiderPortPtr guiderport) {
	...
	int	pause = scale + 2;
	while (1) {
		display(guiderport, scale, 0, 0, 0);
		sleep(pause);
		display(guiderport, 0, scale, 0, 0);
		sleep(pause);
		display(guiderport, 0, 0, scale, 0);
		sleep(pause);
		display(guiderport, 0, 0, 0, scale);
		sleep(pause);
	}
}
...
void	prog1(astro::camera::GuiderPortPtr guiderport) {
	...
	while (1) {
		for (int i = 0; i < 16; i++) {
			...
			display(guiderport, raplus, raminus,
				decplus, decminus);
			sleep(pause);
		}
		sleep(1);
...
void	prog2(astro::camera::GuiderPortPtr guiderport) {
	...
	int	pause = 5 * scale;
	while (1) {
		display(guiderport, 4 * scale, 3 * scale, 2 * scale, 1 * scale);
		sleep(pause);
		display(guiderport, 3 * scale, 2 * scale, 1 * scale, 4 * scale);
		sleep(pause);
		display(guiderport, 2 * scale, 1 * scale, 4 * scale, 3 * scale);
		sleep(pause);
		display(guiderport, 1 * scale, 4 * scale, 3 * scale, 2 * scale);
		sleep(pause);
	}
}
...
void	prog3(astro::camera::GuiderPortPtr guiderport) {
	...
	while (1) {
		display(guiderport, pause,0,0,0);
		sleep(pause);
		display(guiderport, 0,pause,0,0);
		sleep(pause);
	}
}
...
void	prog4(astro::camera::GuiderPortPtr guiderport) {
	...
	while (1) {
		display(guiderport, 0, 0, pause, 0);
		sleep(pause);
		display(guiderport, 0, 0, 0, pause);
		sleep(pause);
	}
}
