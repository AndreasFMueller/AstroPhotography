...
/**
 * \brief main function for the AO test
 */
int	aotest_main(int argc, char *argv[]) {
	...
	// the next argument must be the AO unit name
	if (argc <= optind) {
		throw std::runtime_error("AO unit name missing");
	}
	DeviceName	devicename(argv[optind]);
	std::string	devicestring = devicename;
	debug(LOG_DEBUG, DEBUG_LOG, 0, "adaptive optics test started: %s",
		devicestring.c_str());
